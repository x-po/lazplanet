...
If you go inside the folder, you will see many folders. Go to `FreePascal` folder, then double click `PasLibVlcPlayer.lpk`. It will launch Lazarus with a window titled **Package PasLibVlcPlayer Vx.y.z** (where `x.y.z` is the version number of your downloaded package).
