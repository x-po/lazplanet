...
When we save a project in Lazarus, we don't care about the files that get created, right? We don't need to, because Lazarus handles them in Background. But learning about these files would make you more comfortable about managing multiple projects. And you can also meddle with the project files yourself when you need to do some trick to get things done (as you have to in the {% post_link browser-in-lazarus-with-gecko-pt2 'GeckoPort projects' %}).
...
Superheroes have to save the people from danger. We will now save Lazarus project from losing its bits and bytes from the memory! (i.e. by saving it to hard disk.)
...
The extension `.lpr` is a confusing abbreviation. Because according to **Tools -> Options -> Environment -> File Filters**, `.lpr` means "Lazarus project source" file. It should have been `.lps`, but it is not.
...
When we run a Free Pascal code in the command line we simply save the code as `something.pas` and compile it. This lpr file is similar but with a different extension and some code already put in by Lazarus. It contains the source code just like `.pas` file would do.
...
The extension `.lps` probably means "Lazarus Project Session". This is a very interesting file because at first glance, it looks similar to the `.lpi` file. But it has different purpose. It contains the session data. That means it stores what you did in the last time you opened the project.

For example, if you opened the `Classes` file in a tab (Ctrl+click on the unit name `Classes` in the source editor), Lazarus remembers the next time you open the project and shows the `Classes` tab from your previous session. It even remembers where your cursor was in those files. Pretty amazing! And this is possible because Lazarus stores all these data in this `.lps` session file.
...
Now open the project in Lazarus (by double clicking the lpi or lpr file or using **File -> Open**). Click **Run -> Compile**. (You can also try **Run -> Run**.) When the compiling finishes you will see a `lib` folder to be created in the project folder. It has a `.compiled` and `.o` file. They are the result of compiling.
...
You should also see a **project1.exe** in the project folder which is our executable. This executable is not ready to be published. It has debugging data. To strip off this data check out {% post_link how-to-reduce-exe-file-size-of-your 'Build modes' %}.
...
Go to the folder "Application1" you just created.


![Lazarus project files for a GUI Application project](post-mortem-of-lazarus-project/Lazarus-Project-Post-Mortem-4.gif)


Now there are 7 files! (Compared to only 3 files in the console project.) This is because GUI application needs more data than a console project. It has to have the source code, of course, plus the component properties, resource file and an icon and so on.
...
![Lazarus "paw" icon](post-mortem-of-lazarus-project/Lazarus-Project-Post-Mortem-5.gif)
...
The `Caption` that we set in Lazarus and even the dimensions are saved in this file. (Obviously, the dimensions shown here would not be the same in your project.)
...
So, in both cases of Console and GUI projects, `.lpr` file always stores the main program, and units contain the additional code to aid the main program.
...
Basically, Lazarus invents nothing by itself. Every piece of data about a Lazarus project is stored in a file somewhere. There are separate files for separate things. The forms are saved in LFMs, their code is saved in PASes, when compiling you need compiler options and it is saved in the LPI file and so on... Now that you have the knowledge, you can have a better time with coding your programs.
