...
downloads:
  - https://drive.protonmail.com/urls/T1VNMJV5YR#Gb9PkIdR4DaP
---

Forms are rectangular, right? In this article we prove it wrong. We will turn our form into circle, triangle and what not!
<!-- more -->

Happy _{% post_link shuvo-noboborsho-and-here-is-new-mascot 'Pahela Baishakh' %}_ to all _Bangalees_. Have a good year 1421 (Bengali Year).
...
First time I saw a shaped form was the Sonique Player. It was back in 2001 or '02. I used to use it on Windows 98. I had no internet so I got it from a magazine CD. Its development stopped in 2002. [Wikipedia](https://www.wikipedia.org/) has a [great article about Sonique Player](https://en.wikipedia.org/wiki/Sonique_%28media_player%29). It was a good player with a cool interface. It had a intuitively shaped form. look at the screenshot below (click it to zoom):


![Sonique player with its custom shaped form](shape-your-form-into-anything/Media_player_sonique-1.95.png)
...
It's that easy! Oh, and you can use it on any component as well. For example, a button. Such an example project already exists in your Lazarus installation. Check out `C:\lazarus\examples\shapedcontrols` (assuming that you have Lazarus installed in `C:\lazarus`).
...
If you load a Monochrome bitmap file with a complex drawing in this Canvas you could even make complex of shapes of the form, such as the shape of the [Sonique Player](http://en.wikipedia.org/wiki/Sonique_(media_player)).
...
Now draw 5 **TButton**s in the form. Set their **Caption**s as:
- Circle
- Rounded Rectangle
- Triangle
- Bitmap
...
The **TButton**s will be named `Button1` through `Button5`. The code is given here assuming this. You may have to modify the code if it is not correct.
...
We have opened 'cloud.bmp'. So how did I make this file. You can use any Image editor for this. Create a new image with the same width and height as the form's. Fill it with Black. Draw shapes in white color. I drew a cloud shape. You can also use the image below instead if you want:


![Form mask for Lazarus project](shape-your-form-into-anything/cloud.bmp "Form mask for Lazarus project")


To get the above image in your computer right click it and select **Save Image As...** and save it where you saved your project. If you haven't saved it, save the project (File -> Save all). Otherwise, the button won't work.
...
![Lazarus custom form shape: Original form shape restored](shape-your-form-into-anything/form-shape-lazarus-2.gif)
...
Setting the `BorderStyle` to `bsNone` makes the form immovable with dragging the mouse. For this, you can try {% post_link dragging-form-by-its-body 'this method described in this article' %}. It will make your form movable.
