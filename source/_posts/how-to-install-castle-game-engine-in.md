...
![A first person shooter game made with Castle](how-to-install-castle-game-engine-in/castle_screen_demo_1.jpg "FPS game in Castle (Lazarus)")
...
![A 3D file viewer, view3dscene, made with Castle](how-to-install-castle-game-engine-in/castle_sunset.jpg)
...
You will also need disk space to store the downloaded files and rebuild Lazarus. You also need decent hardware requirements to work with 3D environment. It depends on the work you will do. But if you have a 1.5 Ghz processor, 1GB RAM and a 512 MB graphics card you are pretty much safe with basic projects.
...
Inside the file you will find a "`castle_game_engine`" folder. You can extract it to anywhere. But I like to keep it in the `components` folder of my Lazarus installation. So I have extracted it to **C:\lazarus\components** directory, so I have a directory like this `C:\lazarus\components\castle_game_engine\src`.
...
3\. Select the 3 files: **castle_base.lpk**, **castle_components.lpk**, **castle_window.lpk** and click **Open**. Now 3 windows will appear one over the other. Move the windows to see each of the windows.
...
You will see in the messages window that the "Compiling package xyz completed". Do this for **all the 3 windows.**
...
1\. Go to **C:\lazarus\components\castle_game_engine\examples\lazarus\model_3d_viewer**. (Or if you have extracted elsewhere then **castle\_game\_engine\examples\lazarus\model\_3d\_viewer**)
...
You will notice that the generated EXE is so big (around 37 mb). You can {% post_link how-to-reduce-exe-file-size-of-your 'reduce EXE file size by following a tutorial article here' %}. By following the tutorial you can reduce size with 2 clicks, or even none.
