...
begin
  SystemParametersInfo(SPI_SETDESKWALLPAPER, 0, pchar(FileNameEdit1.FileName), SPIF_SENDWININICHANGE);
end;
```
...
Draw 2 `TSpinEdit`s (from **Misc** tab). Name one `spinX` and another `spinY`. Set both of their `Value` and `MinValue` property to `1`. Don't worry about the `MaxValue`. We'll set their `MaxValue` on Form's `onCreate`. So, double click on the form and enter:
